...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, isDevMode, setIsDevMode }) => {
    ...
    const handleSave = async () => {
        if (displayName.trim() && user && displayName.trim() !== user.displayName) {
            setIsSaving(true);
            setError(null);
            try {
                                        ...
                                        <div className="absolute top-14 right-0 z-50 w-56 bg-[#1a0a37]/90 backdrop-blur-lg border border-[#00f2ff]/30 rounded-lg shadow-2xl shadow-[#00f2ff]/20 p-2 animate-fade-in">
                                            <div className="flex items-center gap-3 p-2 border-b border-[#00f2ff]/20 mb-2">
                                                ...
                                                <div className="flex-1">
                                                    <p className="font-bold text-white truncate">{currentUser.displayName || currentUser.username || 'User'}</p>
                                            ...
                                            <button data-testid="logout-button" onClick={() => { logout(); setIsMenuOpen(false); }} title="Reset App" className="w-full flex items-center gap-3 px-3 py-2 text-left text-sm text-red-400 rounded-md hover:bg-red-500/20 transition-colors">
