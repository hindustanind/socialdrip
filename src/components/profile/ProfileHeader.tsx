...
interface ProfileHeaderProps {
  user: User;
  onUpdateProfile: (updates: Partial<Pick<User, 'displayName' | 'styleSignature'>>) => Promise<void>;
  onUpdateAvatar: (file: File) => Promise<void>;
  isOwnProfile: boolean;
}

const ProfileHeader: React.FC<ProfileHeaderProps> = ({ user, onUpdateProfile, onUpdateAvatar, isOwnProfile }) => {
  ...
  const handleSaveSignature = async () => {
    ...
    setError(null);
    try {
        await onUpdateProfile({ styleSignature: newSignature });
        setIsEditing(false);
    } catch(err: any) {
      ...
      <Modal isOpen={isEditing} onClose={() => setIsEditing(false)} title="Edit Your Style Signature">
        <div className="p-4 flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-4">
            <Button onClick={() => setIsEditing(false)} variant="secondary" disabled={isSaving}>Cancel</Button>
