...
const PoseDeck: React.FC<PoseDeckProps> = ({ poses, selectedId, onSelect, disabled, size = 'small' }) => {
    const dragStartX = useRef<number | null>(null);
    const deckRef = useRef<HTMLDivElement>(null);
    const [isAutoScrolling, setIsAutoScrolling] = useState(true);

    const commandedIndex = useMemo(() => {
        const index = poses.findIndex(p => p.id === selectedId);
    ...
    // Auto-scroll effect
    useEffect(() => {
        if (isAutoScrolling && !disabled && poses.length > 1) {
            const intervalId = window.setInterval(() => {
                setCurrentIndex(prevIndex => (prevIndex + 1) % poses.length);
            }, 3000); // Auto-scroll every 3 seconds
            return () => clearInterval(intervalId);
        }
    }, [isAutoScrolling, disabled, poses.length]);
    ...
    const stopAutoScrollAndSelect = (newIndex: number) => {
        ...
        if (poses[newIndex] && poses[newIndex].id !== selectedId) {
            onSelect(poses[newIndex].id);
        }
    };

    const handleNavigation = (direction: 'next' | 'prev') => {
        if (disabled || poses.length <= 1) return;
    ...
    const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        e.preventDefault();
        const threshold = e.currentTarget.offsetWidth / 5;
    ...
    return (
        <div className="relative w-full h-full flex flex-col items-center justify-center">
                        ...
                        return (
                            <div
                                ...
                                style={{
                                    ...
                                    zIndex: poses.length - Math.abs(offset),
            ...
            <div className="flex items-center gap-4 mt-auto pt-4">
                <button onClick={() => handleNavigation('prev')} disabled={disabled || poses.length <= 1} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors disabled:opacity-50">
                ...
                <span className="text-lg font-semibold">{poses.length > 0 ? currentIndex + 1 : 0} / {poses.length}</span>
                <button onClick={() => handleNavigation('next')} disabled={disabled || poses.length <= 1} className="p-2 rounded-full bg-white/10 hover:bg-white/20 transition-colors disabled:opacity-50">
