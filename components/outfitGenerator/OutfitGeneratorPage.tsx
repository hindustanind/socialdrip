...
// FIX: Define the props interface for the component to resolve the type error.
interface OutfitGeneratorPageProps {
  onOutfitSaved: (newOutfit: Omit<Outfit, 'id' | 'createdAt'>) => void;
  isDevMode: boolean;
  userName: string;
...
const OutfitGeneratorPage: React.FC<OutfitGeneratorPageProps> = ({ onOutfitSaved, isDevMode, userName, styleSignature, outfits, showToast }) => {
    ...
    // This is the core state machine driver
    ...
    useEffect(() => {
        ...
        if (flowStatus === 'loading' && currentAngle && angleStatuses[currentAngle] === 'generating') {
            ...
            if (!baseImage) return; // Should not happen if logic is correct
            
            const base64Image = generationState.images['Front'] || selectedFile!.name; // This is a placeholder for dev mode
    ...
    const handleGenerationStart = (croppedImageDataUrl: string) => {
        ...
        setSelectedFile(new File([], base64Image)); // Store the base64 string in a dummy file name for dev mode access

        setGenerationState({
            flowStatus: 'loading',
            currentAngle: 'Front',
            images: { 'Front': isDevMode ? base64Image : undefined }, // Pre-fill for dev mode
    ...
    const handleSave = async () => {
        ...
        try {
            // DUPLICATE CHECK
            if (!isDevMode && geminiService.isApiKeySet() && outfits.length > 0) {
                setSaveStatus('checking');
                const newImageBase64 = generatedOutfit.images[0];
                const existingImages = outfits
                    .map(o => ({ id: o.id, base64: (o.originalImages && o.originalImages[0]) || o.images[0] }))
                    .filter(o => o.base64);

                if (existingImages.length > 0) {
                    const duplicateId = await geminiService.findDuplicateImage(newImageBase64, existingImages);
                    if (duplicateId) {
                        showToast("You already have this outfit in your closet!", 'error');
                        setSaveStatus('idle');
            ...
            console.error("Failed to check for duplicates or save outfit:", err);
            if (err instanceof geminiService.QuotaExceededError) {
                showToast(`Hey ${userName}! ✨ Your closet is looking amazing! You've reached the daily limit for organizing. Please come back tomorrow to save more.`, 'error');
                setGenerationState(prev => ({...prev, error: 'Daily limit reached.'}));
            } else {
    ...
    const getSaveButtonText = () => {
        if (saveStatus === 'checking') return 'Checking...';
        ...
        if (flowStatus === 'loading') {
            return (
                <div key="loading" className="page-transition-enter flex items-center justify-center min-h-[calc(100vh-11rem)]">
                    {/* FIX: Updated GenerationLoader props to be compliant with its new interface. */}
                    <GenerationLoader
